...
# Usage: ./unbind_from_driver.sh <ssss:bb:dd.f> <retries>
...
{
   local gpu=$1
   local lock_retries=$2
   local unbindlock_file="/proc/driver/nvidia/gpus/$gpu/unbindLock"
   local unbindlock=0
   local attempt=1

   if [ ! -e "$unbindlock_file" ]; then
      echo "unbindLock file does not exist for $gpu" >&2
      return 1
   fi
...
unbind_from_driver()
{
   ...
   if [ "$existing_driver_name" == "nvidia" ]; then
      acquire_unbind_lock $gpu $lock_retries || return 1
   fi
   echo "$gpu" > "$existing_driver/unbind"
   return 0
}

unbind_from_driver $1 $2 || exit 1