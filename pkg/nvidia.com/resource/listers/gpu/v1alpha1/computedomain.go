...
type ComputeDomainLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.ComputeDomain, err error)
...
// List lists all ComputeDomains in the indexer.
func (s *computeDomainLister) List(selector labels.Selector) (ret []*v1alpha1.ComputeDomain, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.ComputeDomain))
	})
	return ret, err
}
...
type ComputeDomainNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.ComputeDomain, error)
...
// List lists all ComputeDomains in the indexer for a given namespace.
func (s computeDomainNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.ComputeDomain, err error) {
...
// Get retrieves the ComputeDomain from the indexer for a given namespace and name.
func (s computeDomainNamespaceLister) Get(name string) (*v1alpha1.ComputeDomain, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("computedomain"), name)
	}
	return obj.(*v1alpha1.ComputeDomain), nil
}
