...
import (
	...
	nvcrd "github.com/NVIDIA/k8s-dra-driver/pkg/nvidia.com/api/resource/gpu/v1alpha1"
)
...
type RequestedGpus = nvcrd.RequestedGpus
type RequestedMigDevices = nvcrd.RequestedMigDevices
...
func (g *NodeAllocationState) Create() error {
	...
	crd, err := g.clientset.GpuV1alpha1().NodeAllocationStates(g.Namespace).Create(context.TODO(), crd, metav1.CreateOptions{})
	if err != nil {
		return err
	}
...
func (g *NodeAllocationState) Delete() error {
	...
	err := g.clientset.GpuV1alpha1().NodeAllocationStates(g.Namespace).Delete(context.TODO(), g.NodeAllocationState.Name, deleteOptions)
...
func (g *NodeAllocationState) Update(spec *nvcrd.NodeAllocationStateSpec) error {
	crd := g.NodeAllocationState.DeepCopy()
	crd.Spec = *spec
	crd, err := g.clientset.GpuV1alpha1().NodeAllocationStates(g.Namespace).Update(context.TODO(), crd, metav1.UpdateOptions{})
	if err != nil {
		return err
	}
	*g.NodeAllocationState = *crd
...
func (g *NodeAllocationState) Get() error {
	crd, err := g.clientset.GpuV1alpha1().NodeAllocationStates(g.Namespace).Get(context.TODO(), g.Name, metav1.GetOptions{})
	if err != nil {
		return err
	}
	*g.NodeAllocationState = *crd
